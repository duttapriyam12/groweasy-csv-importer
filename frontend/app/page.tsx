...
export default function Home() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-10 sm:px-8">
        ...
        <section>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-ink/50">{previewRows.length} rows detected — raw preview, no AI applied yet</p>
            </div>
      ...
      {step === "processing" && (
        <section className="flex flex-col items-center justify-center rounded-2xl border border-ink/10 bg-white py-20">
          <div className="mb-4 h-10 w-10 animate-spin rounded-full border-4 border-accent/20 border-t-accent" />
          <p className="font-display font-semibold text-ink">Mapping your leads with AI…</p>
        ...
        <div key={s.key} className="flex items-center gap-2">
          <div
            className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-semibold ${
              i <= activeIndex ? "bg-accent text-white" : "bg-ink/10 text-ink/40"
            }`}
          >
            {i + 1}
          </div>
          <span className={`text-xs ${i <= activeIndex ? "text-ink" : "text-ink/40"}`}>{s.label}</span>
